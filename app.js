...
const AUTO_CENTER_EVERY_MS = 60_000; // 1 minuto (pon tu X aquí)
...
function locateUser() {
  ...
  // 1) Siempre actualiza el marcador de "yo" (esto NO mueve el mapa)
  updateUserMarker(ll);

  // 2) Solo auto-centrar cada X tiempo
  const now = Date.now();
  if (now - lastAutoCenterAt >= AUTO_CENTER_EVERY_MS) {
