...
// --- SECTORES ---
let selectedSectors = new Set();   // vacío = equivale a "Todos"
let allSectors = [];               // lista ordenada de sectores disponibles

function normalizeSector(sector) {
  const s = (sector ?? "").toString().trim();
  return s.length ? s : "Sin sector";
}


function sectorMatches(falla) {
  // Si no hay selección específica => equivale a "Todas"
  if (selectedSectors.size === 0) return true;

  const s = normalizeSector(falla.sector);
  return selectedSectors.has(s);
}
// ------------------ Persistencia ------------------
...
// ------------------ Inicio / Fin manual ------------------
let startMarker = null;   // Inicio fijo (manual)
let endMarker = null;     // Fin fijo (manual)
let endMode = "none";     // "none" | "manual"
...
function enablePickEndMode() {
  pickEndMode = true;
  pickStartMode = false;
  alert("Toca el mapa donde quieras poner el FIN de la ruta (opcional).");
  updateEndButtonText();
}

function setEndPoint(lat, lng) {
  if (endMarker) map.removeLayer(endMarker);
  endMarker = L.marker([lat, lng]).addTo(map).bindPopup("🏁 Fin de ruta").openPopup();
  pickEndMode = false;
  endMode = "manual";
  updateEndButtonText();
}
...
function getOriginAuto() {
  // 1) Inicio manual
  if (startMarker) {
    const ll = startMarker.getLatLng();
    return { lat: ll.lat, lng: ll.lng, label: "🎯 Inicio elegido" };
  }

  // 2) GPS
  if (userMarker) {
    const ll = userMarker.getLatLng();
    return { lat: ll.lat, lng: ll.lng, label: "📍 Mi ubicación" };
  }
...
function getDestinationIfAny() {
  if (endMode === "none") return null;

  if (endMode === "manual" && endMarker) {
    const ll = endMarker.getLatLng();
    return { lat: ll.lat, lng: ll.lng, label: "🏁 Fin elegido" };
  }
...
function updateEndButtonText() {
  const btn = document.getElementById("endGpsBtn");
  if (!btn) return;

  if (endMode === "manual" && endMarker) {
    btn.textContent = "🏁 Fin fijo ✅ (limpiar)";
  } else if (pickEndMode) {
    btn.textContent = "🏁 Toca el mapa…";
  } else {
    btn.textContent = "🏁 Elegir fin";
  }
}
...
function buildSectorFilter(fallas) {
  const listEl = document.getElementById("sector_list");
  const allCb = document.getElementById("sector_all");

  // Opcional (si quieres igual que Secciones, tipo desplegable)
  const btn = document.getElementById("sectorsToggleBtn");
  const panel = document.getElementById("sectorsPanel");

  if (!listEl || !allCb) return;

  const sectors = Array.from(new Set(fallas.map((f) => normalizeSector(f.sector))))
    .filter(Boolean);

  // Orden: alfabético, pero "Sin sector" al final
  sectors.sort((a, b) => a.localeCompare(b, "es"));
  const idx = sectors.indexOf("Sin sector");
  if (idx !== -1) {
    sectors.splice(idx, 1);
    sectors.push("Sin sector");
  }
  ...
  for (const sec of sectors) {
    const row = document.createElement("label");
    row.className = "sec-item"; // reutiliza estilo de secciones si existe
    row.innerHTML = `
      <input type="checkbox" class="sector-cb" data-sector="${sec}" checked />
      <span>${sec}</span>
    `;
    listEl.appendChild(row);
  }

  // Si tu CSS oculta el contenedor, fuerzo visibilidad mínima
  listEl.style.display = listEl.style.display || "block";

  // Toggle opcional (si existe)
  if (btn && panel) {
   btn.addEventListener("click", (e) => {
  e.stopPropagation();
  ...
  allCb.addEventListener("change", () => {
    ...
    updateSectorsButtonText();
    applyFilters();
  });

  // Cambio en cualquier sector
  listEl.addEventListener("change", (e) => {
    if (!e.target.classList.contains("sector-cb")) return;

    const cbs = Array.from(document.querySelectorAll(".sector-cb"));
    const allChecked = cbs.every((cb) => cb.checked);
    const noneChecked = cbs.every((cb) => !cb.checked);

    // Evitar “ninguno seleccionado”: vuelve a todos
    if (noneChecked) {
      cbs.forEach((cb) => (cb.checked = true));
      allCb.checked = true;
    } else {
  ...
  function updateSectorsButtonText() {
    if (!btn) return;
    const selected = getSelectedSectors();
    btn.textContent = selected === "ALL" ? "Sectores (todos)" : `Sectores (${selected.length})`;
  }
...
function getSelectedSectors() {
  ...
  const selected = cbs.filter((cb) => cb.checked).map((cb) => cb.dataset.sector);
  if (selected.length === cbs.length) return "ALL";
  return selected;
}
...
function applyFilters(clearRouteToo = true) {
  ...
  if (selectedSectors !== "ALL") {
    ...
    filtered = filtered.filter((f) => set.has(normalizeSector(f.sector)));
  }

  // Búsqueda
  if (q) {
    filtered = filtered.filter((f) => {
      const haystack = normalize(`${f.name} ${f.falla_number} ${f.section} ${normalizeSector(f.sector)}`);
...
function buildOptimizedRouteWithDestination(origin, destination, points, maxMeters) {
  ...
  while (remaining.length) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      const stepDist = distanceMeters(current, candidate);
      const toEndDist = distanceMeters(candidate, destination);

      // Distancia total si añado este punto ahora (incluyendo llegar al fin)
      const projectedTotal = usedMeters + stepDist + toEndDist;

      // Si hay presupuesto, no me puedo pasar
      if (Number.isFinite(maxMeters) && maxMeters > 0 && projectedTotal > maxMeters) {
        continue;
      }

      // Heurística: minimiza (paso + lo que me deja para llegar al fin)
      const score = stepDist + toEndDist;
    ...
    // Ningún candidato cabe sin pasarnos del presupuesto
    if (bestIdx === -1) break;

    const next = remaining.splice(bestIdx, 1)[0];
    route.push(next);
    usedMeters += bestStepDist;
    current = next;
  }

  // Añade el tramo final (desde el último punto al destino)
  const lastLeg = route.length ? distanceMeters(current, destination) : distanceMeters(origin, destination);
  const totalMeters = usedMeters + lastLeg;
...
function buildOptimizedRoute(origin, points, maxMeters) {
  ...
  while (remaining.length) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      // Si hay presupuesto y añadir este punto se pasa, lo ignoramos
      if (Number.isFinite(maxMeters) && maxMeters > 0 && usedMeters + d > maxMeters) continue;
  ...
  return { route, totalMeters: usedMeters };
}

function drawRoute(origin, route, destination = null) {
  clearRoute();
...
function buildRoute() {
  const origin = getOriginAuto();
  if (!origin) {
    alert("Necesito un inicio: activa 📍 Mi ubicación o elige 🎯 inicio en el mapa.");
    return;
  }

  const { minutes, kmh, maxMeters } = getRouteBudgetMeters();
  const ignoreVisited = shouldIgnoreVisitedForRoute();
  const destination = getDestinationIfAny();

  const state = loadState();

  // Puntos visibles (ya filtrados por el mapa, porque 'markers' se recrea en renderMarkers)
  ...
  // Si el usuario ha activado un fin pero aún no está definido, avisamos
  if (endMode !== "none" && !destination) {
    alert("Selecciona un fin en el mapa (🏁 Elegir fin) o quita el fin fijo.");
    return;
  }
  ...
  if (!destination) {
    // ✅ Sin fin fijo: comportamiento anterior (ruta libre)
    const res = buildOptimizedRoute(origin, points, maxMeters);
    route = res.route;
    totalMeters = res.totalMeters;
    drawRoute(origin, route);
  } else {
    // ✅ Con fin fijo: reservar distancia para acabar en destination
    const res = buildOptimizedRouteWithDestination(origin, destination, points, maxMeters);
    route = res.route;
    totalMeters = res.totalMeters;
    drawRoute(origin, route, destination);
  }

  const metersPerMin = (kmh * 1000) / 60;
  const estMin = Math.max(1, Math.round(totalMeters / metersPerMin));
...
` +
      `• Inicio: ${origin.label}
` +
      `• Fin: ${destination ? destination.label : "Sin fin fijo"}
` +
      `• Paradas: ${route.length}
` +
      `• Ignorar visitadas: ${ignoreVisited ? "Sí" : "No"}
` +
      `• Distancia aprox: ${Math.round(totalMeters)} m
` +
      `• Tiempo estimado: ~${estMin} min (de ${minutes} min)`
  );
}
...
function renderSectorFilter(sectors) {
  const listEl = document.getElementById("sector_list");
  const allEl = document.getElementById("sector_all");
  ...
  // Crea checkboxes por sector
  for (const sector of sectors) {
    const id = `sector_${sector.toLowerCase().replace(/\s+/g, "_").replace(/[^\w\-]/g, "")}`;

    const label = document.createElement("label");
    label.className = "filter-item";
    label.innerHTML = `
      <input type="checkbox" class="sector_cb" value="${sector}" id="${id}">
      <span>${sector}</span>
    `;
  ...
  allEl.addEventListener("change", () => {
    if (allEl.checked) {
      selectedSectors.clear();
      listEl.querySelectorAll(".sector_cb").forEach(cb => cb.checked = false);
    }
    applyFilters(); // <- tu función central de filtrado
  });

  // Individuales
  listEl.addEventListener("change", (e) => {
    const cb = e.target;
    if (!cb.classList.contains("sector_cb")) return;

    const sector = cb.value;

    if (cb.checked) selectedSectors.add(sector);
    else selectedSectors.delete(sector);

    // Si hay alguno marcado => "Todas" off. Si ninguno => "Todas" on.
...
document.getElementById("endGpsBtn")?.addEventListener("click", () => {
  // Si ya hay fin fijo, el botón actúa como "limpiar"
  if (endMode === "manual" && endMarker) {
    clearEndPoint();
    return;
  }
  // Si estamos en modo selección, cancelar
  if (pickEndMode) {
    pickEndMode = false;
    updateEndButtonText();
    return;
  }
  // Entrar en modo "elegir fin"
  enablePickEndMode();
});
